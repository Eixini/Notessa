...
# Part of the code responsible for checking and working with the directory for storing notes
    def notes_location_directory_checker(self):
        ...
        if self._application_name is None:
            app_path = f'{self.noteLocation}{QDir.separator()}Notessa'
        else:
            app_path = f'{self.noteLocation}{QDir.separator()}{self._application_name}'
    ...
    def text_notes_directory_checker(self):
        ...
        if not QDir(text_notes_path).exists():
            note_dir.mkdir(text_notes_path)
    ...
    def voice_notes_directory_checker(self):
        ...
        if not QDir(voice_notes_path).exists():
            note_dir.mkdir(voice_notes_path)
    ...
    def video_notes_directory_checker(self):
        ...
        if not QDir(video_notes_path).exists():
            note_dir.mkdir(video_notes_path)
    ...
    def paint_notes_directory_checker(self):
        ...
        if not QDir(paint_notes_path).exists():
            note_dir.mkdir(paint_notes_path)
    ...
    def todo_notes_directory_checker(self):
        ...
        if not QDir(todo_notes_path).exists():
            note_dir.mkdir(todo_notes_path)
    ...
    def size_notes_on_disk(self) -> str:
        ...
        if self._application_name is None:
            dir_iter = QDirIterator(f'{self.notes_directory_checker()}', flags=QDirIterator.IteratorFlag.Subdirectories)
        ...
        while dir_iter.hasNext():
            dir_iter.next()
            print(dir_iter.fileInfo().dir().dirName())
            if dir_iter.fileInfo().dir().dirName() == 'TextNotes':
                print(dir_iter.fileInfo().baseName())
                if dir_iter.fileInfo().isFile():
                    count_and_size['total']['size'] += dir_iter.fileInfo().size()
                    count_and_size['total']['count'] += 1
                    count_and_size['text']['size'] += dir_iter.fileInfo().size()
                    count_and_size['text']['count'] += 1
            if dir_iter.fileInfo().dir().dirName() == 'VoiceNotes':
                print(dir_iter.fileInfo().baseName())
                if dir_iter.fileInfo().isFile():
                    count_and_size['total']['size'] += dir_iter.fileInfo().size()
                    count_and_size['total']['count'] += 1
                    count_and_size['voice']['size'] += dir_iter.fileInfo().size()
                    count_and_size['voice']['count'] += 1
            if dir_iter.fileInfo().dir().dirName() == 'VideoNotes':
                print(dir_iter.fileInfo().baseName())
                if dir_iter.fileInfo().isFile():
                    count_and_size['total']['size'] += dir_iter.fileInfo().size()
                    count_and_size['total']['count'] += 1
                    count_and_size['video']['size'] += dir_iter.fileInfo().size()
                    count_and_size['video']['count'] += 1
            if dir_iter.fileInfo().dir().dirName() == 'PaintNotes':
                print(dir_iter.fileInfo().baseName())
                if dir_iter.fileInfo().isFile():
                    count_and_size['total']['size'] += dir_iter.fileInfo().size()
                    count_and_size['total']['count'] += 1
                    count_and_size['paint']['size'] += dir_iter.fileInfo().size()
                    count_and_size['paint']['count'] += 1
            if dir_iter.fileInfo().dir().dirName() == 'TodoNotes':
                print(dir_iter.fileInfo().baseName())
                if dir_iter.fileInfo().isFile():
                    count_and_size['total']['size'] += dir_iter.fileInfo().size()
                    count_and_size['total']['count'] += 1
                    count_and_size['todo']['size'] += dir_iter.fileInfo().size()
                    count_and_size['todo']['count'] += 1

        for key, value in count_and_size.items():
            count_and_size[key] = self.convert_size(count_and_size[key])

        return count_and_size

    def convert_size(self, val: dict):
        print(val)
        # Byte to KByte
